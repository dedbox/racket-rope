...
;; A rope over the same content as (bytes-chunk->rope s), but built by
...
;; chunk->rope would have produced for the same bytes.
(define (fragmented-rope-of s)
  (define n (bytes-length s))
  (define piece-size 37) ;; coprime-ish with 512 on purpose
  (for/fold ([acc (make-empty-bytes-rope)])
            ([start (in-range 0 n piece-size)])
    (define end (min n (+ start piece-size)))
    (bytes-rope-append2 acc (bytes->rope (subbytes s start end)))))
...
(define (rewrapped-rope-of a)
  (and (rope-node? a)
       (bytes-rope-concat (rope-node-left a) (rope-node-right a))))

(define (differing-bytes s pos)
  (define c (bytes-ref s pos))
  (bytes-append (subbytes s 0 pos)
                (bytes (if (= c 97) 98 97))
                (subbytes s (add1 pos))))
...
(module+ main
  ...
  (for ([n (in-list SIZES)])
    (define s (make-bytes n 97))

    (define a (bytes->rope s))
    ...
    (define t-differ-start
      (let ([b (bytes->rope (differing-bytes s 0))])
        (bench-min (λ () (equal? a b)))))

    (define t-differ-end
      (let ([b (bytes->rope (differing-bytes s (sub1 n)))])
        (bench-min (λ () (equal? a b)))))
