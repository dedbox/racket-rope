...
(define (implementation-A split append2 ->rope a i k chunk)
  (let*-values ([(before rest) (split a i)]
                [(_gone after) (split rest k)])
    (append2 (append2 before (->rope chunk)) after)))

(define (implementation-B a0 p0 elem-size)
  (if (number? elem-size)
...
(define (run-benchmark)
  ...
  (for* ([type   (in-list '(string token))]
    ...
    (define-values (split append2 ->rope)
      (if (eq? type 'string)
          (values string-rope-split string-rope-append2 string->rope)
          (values tokens-rope-split tokens-rope-append2 tokens->rope)))
    ...
    (for ([pos-pair (in-list sweeps)])
      ...
      (for ([_ (in-range 10000)])
        (implementation-A split append2 ->rope a0 p0 elem-size)
        (implementation-B a0 p0 elem-size))
      
      ;; 2. Strict Measurement
      (define a-ms (measure-time (λ () (implementation-A a0 p0 elem-size)) ITERATIONS TRIALS))
      (define b-ms (measure-time (λ () (implementation-B a0 p0 elem-size)) ITERATIONS TRIALS))
