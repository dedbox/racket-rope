...
(define (fragmented-rope-of s)
  ...
  (for/fold ([acc (make-empty-string-rope)])
    ...
    (string-rope-append2 acc (string->rope (substring s start end)))))
...
(module+ main
  ...
  (for ([n (in-list SIZES)])
    ...
    (define t-differ-start
      (let ([b (string->rope (differing-string s 0))])
        (bench-min (λ () (equal? a b)))))

    (define t-differ-end
      (let ([b (string->rope (differing-string s (sub1 n)))])
        (bench-min (λ () (equal? a b)))))
